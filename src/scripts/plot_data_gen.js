...
请记得为最有趣的选项添加[选择]标记。故事应反映角色的过去经历，并与故事风格主题（道德挣扎、生存与救赎、科技与人性）保持一致。`,
  ...
  ENDING: `扮演一位资深的文字游戏作者，请你根据之前发生的故事和玩家做出的选择和结果，并参考故事的风格倾向，为角色继续讲述，在故事的开头你需要简单交代前情并继续描述后续的发展，并为玩家生成三个可以选择的行动，每个行动后用括号标注这个选择会需要使用玩家的哪项属性，可以选择的属性包括无、力量、敏捷、体质、感知、魅力、智力。当前是剧情的结局部分，你应该根据之前的故事，为当前的剧情逐步收尾。如果你根据上文觉得故事适合结束，你可以结束这个故事，并且不必再生成选项而是给出结局，如果你确定结束故事你需要末尾加上"故事终"。
...
function getSystemPromptForRound(round) {
  if (round >= PHASES.ENDING.start) {
    return SYSTEM_PROMPTS.ENDING;
  } else if (round >= PHASES.DEVELOPMENT.start) {
    return SYSTEM_PROMPTS.DEVELOPMENT;
  } else {
    return SYSTEM_PROMPTS.BEGINNING;
  }
}
