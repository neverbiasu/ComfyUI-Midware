...
if (args.length > 0) {
  ...
  if (!isNaN(roundArg) && roundArg >= 0 && roundArg < BATCH_SIZE) {
    ...
    console.error(
      `无效的起始轮次: ${args[0]}. 必须在 0 和 ${BATCH_SIZE - 1} 之间`
    );
    process.exit(1);
  }
}
...
暴雨夜，你在废弃的圣玛丽医院寻找失踪的妹妹。手电筒光束扫过三楼妇产科锈迹斑斑的走廊时，突然照见墙角的血迹组成了妹妹手链上的鸢尾花纹样。通风管道传来婴儿啼哭般的风声，你发现血迹尽头的地砖刻着倒五芒星符号，缝隙里渗出暗绿色黏液。
...
async function batchGeneratePlots() {
  ...
  if (startRound > 0) {
    ...
    if (fs.existsSync(previousRoundFile)) {
      ...
      // 解析上一轮的选择
      const { fullText, selectedChoice } = parseStoryAndChoices(
        previousContent,
        startRound - 1
      );
      ...
      if (selectedChoice) {
        ...
        console.log(
          `已加载第 ${startRound} 轮内容，选择了选项 ${selectedChoice}`
        );
      } else {
