...
const plotImageGenRouter = Router();
...
const workflowJson = fs.readFileSync(
  "./src/assets/plot_image_gen.json",
  "utf-8"
);
...
plotImageGenRouter.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Update character description (Node 16)
      workflow["16"].inputs.value = characterDescription;

      // Update portrait image (Node 7)
      workflow["7"].inputs.image = portraitImageName;
      ...
      workflow["13"].inputs.seed = parseInt(randomSeed());
...
export default plotImageGenRouter;
